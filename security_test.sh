...
if [ -z "${IMAGE_REF:-}" ]; then
  echo "==> [0/4] Building bff-dashboard:local from development.Dockerfile..."
  docker build -f development.Dockerfile -t bff-dashboard:local \
    --secret id=npmrc,src=.npmrc \
    --secret id=node_auth_token,env=NODE_AUTH_TOKEN \
    . || exit 1
  export IMAGE_REF="bff-dashboard:local"
fi
